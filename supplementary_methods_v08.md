...
Zenodo DOI 10.5281/zenodo.22865462 preserves the released derived model inputs, random-score matrices, expected outputs, R script, tables and original figure exports. The GitHub companion provides the directory-preserving working copy. The package starts from derived analysis inputs; downloading and rebuilding every original expression matrix is outside its executable scope. Figure files supplied with this manuscript were regenerated from the same frozen result tables for PLOS ONE formatting. Supplementary table values remain those of the frozen analysis, with the reference-verification record updated for this submission review.

S2 Text supplies a SHA256 manifest for the 15 released input and expected-output files. After extracting the Zenodo archive, save S2 Text as manifest.json inside its reproducibility directory. Run python verify_inputs.py from that directory; a successful check reports 15 checked files and no mismatches. The manifest applies to the exact archive bytes, including line endings.
